...
그런데, Collection 을 확장하고 있다..! 순서를 잘못 잡은 듯 하다.<br/>
Collection 부터 살펴보자.
...
그런데, Iterable 을 확장하고 있다.. 역시나 순서를 잘못 잡은 듯 하다.
Iterable 로 이동해보자.
...
Iterable 인터페이스를 구현하는 것은 객체가 for-each loop 선언문의 타겟이 되도록 하는 것이라고 한다.
...
아직 `Iterator`를 살펴보지는 않았지만, 반복을 수행하는 자료구조다 라고 생각하고 일단 봐보자.
...
`Collection` 은 객체 그룹을 표현하는데, 어떤 컬렉션들은 중복된 요소를 허용하기도 하고, 일부는 그렇지 않기도 하다. 또 어떤 것들은 순서가 있고, 일부는 없다.<br/>
...
> 이러한 기능이 제공될 수 있는 이유는 `List` 가 순서가 정해져있는 컬렉션이기 때문이 아닐까?<br/>
> 순서가 정해져 있지 않은 컬렉션이라는 개념이 잘 이해가 되지 않기 때문에 이 부분이 아직 모호하다.
> 추측하기로는 인터페이스 설계상으로는 순서가 정해져 있지 않은 컬렉션이 '이전 요소' 에 대한 접근을 할 수 있는 것이 모순? 이기에 제공하지 않는 것이 아닐까 싶다. (반면 반복할 수 있는(iterable) 요소들의 집합이 컬렉션이라는 점에서 '다음 요소'를 알아내는 것에는 개념적으로 문제가 없다.)<br/>
